...
public class FarmersListActivity extends AppCompatActivity {
    ...
    private double latitude_end, longitude_end;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        DocumentReference noteref3 = db.collection("Users").document(FirebaseAuth.getInstance().getCurrentUser().getUid());
        noteref3.get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
            @Override
            public void onSuccess(DocumentSnapshot documentSnapshot) {
                latitude_end = documentSnapshot.getDouble("latitude");
                longitude_end = documentSnapshot.getDouble("longitude");
            }
        });
...
//                                    Log.i("CheckUID", currentItem.getPrice());
                                    farmerList2.add(new FarmerItem2(currentItem.getName(), currentItem.getCategory(), currentItem.getPrice(), currentItem.getDate(), currentItem.getTime(),
                                            ...
                                            user_name, address, latitude, longitude, phone, 0, currentItem.getUpi(), latitude_end, longitude_end));
                                }
                            }
                        }
